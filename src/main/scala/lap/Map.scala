...
import util.Random
import scala.Some
import collection.mutable.{HashSet, ArrayBuffer, Queue}
...
class Map(size: Int) {
  ...
  val map = ArrayBuffer.fill(size, size)(0)
  ...
  def slideSequence() {
    ...
    var stepsPassed = 0
    var noChange = 0
    var lastMap = map.clone()
    while (stepsPassed < 50000 && noChange < 20) {
      stepsPassed += 1
...
//      Console.readLine()
    ...
    if (hand != 0) {
      ...
      for (y <- 0 until size) {
        for (x <- 0 until size) {
          if (map(x)(y) == 0) emptyPos = ((x, y))
        }
      }
      if (!doesBreakChain(emptyPos, hand))
        access(emptyPos, hand)
    }
  }
